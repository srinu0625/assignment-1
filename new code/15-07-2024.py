...
# Assigning values of rows for previous candles
...
for index1, row1 in data1.iterrows():
    ...
    try:
        ...
        print("Current High3:", current_high3, "Previous High3:", previous_high3, "Local High3:", local_high3)
        print("Current Low3:", current_low3, "Previous Low3:", previous_low3, "Local Low3:", local_low3)
        time.sleep(0.5)
        print("   ")
        ...
        # Bullish candle
        max_loss_for_trade = (local_high1 - current_low1 + (tick_val * 4)) * 5 * num_of_lots
        if current_high1 > local_high1 and not bull and number_of_positions < max_num_lots:
            number_of_positions += 1
            num_of_trades += 1
            num_of_lots += 1
            entry_price = current_high1
            exit_price = local_low1
            bull = True
            bear = False
            flag = True
            date_flag = False
            print("Long trade initiated at: ", entry_price)
            print("Current time: ", current_time1)
            time.sleep(0.5)
        elif bull and current_high1 < local_high1:
            bull = False

        # Bearish candle
        max_loss_for_trade = (current_high1 - local_low1 + (tick_val * 4)) * 5 * num_of_lots
        if current_low1 < local_low1 and not bear and number_of_positions < max_num_lots:
            number_of_positions += 1
            num_of_trades += 1
            num_of_lots += 1
            entry_price = current_low1
            exit_price = local_high1
            bull = False
            bear = True
            flag = True
            date_flag = False
            print("Short trade initiated at: ", entry_price)
            print("Current time: ", current_time1)
            time.sleep(0.5)
        elif bear and current_low1 > local_low1:
            bear = False
