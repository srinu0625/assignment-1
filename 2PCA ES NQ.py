...
# ----------------------------------------------------------
# when products are highly correlated PCA extracts spreads relationship adn when that relartion deviates Z_ SCORE will trade when it reverts will exit 
# dectet temporary mispricing b\w two products using PCA and make profit from mean reversion
...
# CONFIG
# ==========================
CSV_FILE = r"D:\data 3\ES_NQ 1.csv"   # input data file
OUTPUT_PATH = r"D:\ARB_PROJECT1\ES_NQ PCA_RESULTS 1min.xlsx"  # where results will be saved

ROLLING_WINDOW = 150   # how much past data we use to understand behaviour

ENTRY_Z = 2.5   # enter trade when deviation is big
...
for i in range(ROLLING_WINDOW, len(returns)):
    ...
    mean = pc1.mean() # calculating meanvariance
    std = pc1.std()   # calculating standard deviation 
    if std == 0:
        continue
    ...
    es_w, nq_w = weights
    hedge_ratio = abs((es_w * ES_MULT) / (nq_w * NQ_MULT))
    ...
    if position == 0:
        ...
        if z > ENTRY_Z:
            position = -1
            entry = (date, es_price, nq_price, hedge_ratio, i, "SHORT")
            print(f"{date} | \033[91mSHORT ES LONG NQ\033[0m | Z={z:.2f}")
            time.sleep(10)

        elif z < -ENTRY_Z:
            position = 1
            entry = (date, es_price, nq_price, hedge_ratio, i, "LONG")
    ...
    # ==========================
        ...
        nq_pnl = (nq_price - entry[2]) * NQ_MULT * position * entry[3]
