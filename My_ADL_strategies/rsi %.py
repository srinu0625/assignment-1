...
file_path = r"D:\Data\ES Sep25_1min.csv"
...
for i in range(max(n, rsi_period, 5), len(df)):
    try:
        ...
        # LONG ENTRY
        if close_today > ema and rsi >= rsi_value and rsi_pct > rsi_pct_value and position == 0:
...
# 
        # LONG EXIT
        elif position == 1 and (close_today < ema or rsi <= rsi_value or rsi_pct < rsi_pct_value):
        ...
        # SHORT ENTRY
        elif close_today < ema and rsi <= rsi_value and rsi_pct < rsi_pct_value and position == 0:
            Entry_price = close_today
            Entry_time = date_time
            position = 2
        ...
        # SHORT EXIT
        elif position == 2 and (close_today > ema or rsi >= rsi_value or rsi_pct > rsi_pct_value):
            Exit_price = close_today
            Exit_time = date_time
