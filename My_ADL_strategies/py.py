...
df['EMA_RSI'] = df['RSI'].ewm(span=14, adjust=False).mean()   # EMA of RSI
df['EMA_x_RSI'] = df['ema'] * df['RSI']  # If using multiplication logic
...
for i in range(warmup, len(df)):
    try:
        ...
        if position == 0 and (macd > signal) and (ema_rsi < 50):
            # Optional: use multiplication filter => and (ema_x_rsi < some_threshold)
            position = 1
            entry_price = close
            entry_time  = date
            entry_index = i
            ...
            print(f" RSI          : {rsi:.2f} | EMA_RSI: {ema_rsi:.2f}")
            print(f" ATR          : {atr:.4f}")
            print("================================\n")
            continue
        # time.sleep(0.5)
        ...
        # ------------- LONG EXIT -------------
        if position == 1:
            stop_loss     = entry_price - (atr_mult * atr)   # fixed SL direction
            target_profit = entry_price + (3 * atr)          # fixed TP direction
            if close >= target_profit or (close <= stop_loss) or macd_cross_dn:
                exit_price = close
                ...
                trade_log.append({
                    ...
                    'exit_reason': 'MACD_cross' if macd_cross_dn else 'ATR/TP',
                    'rsi_exit': rsi,
                    'ema_rsi_exit': ema_rsi,
                    'macd_exit': macd,
                    'signal_exit': signal,
                    'atr_exit': atr
                })
        ...
        if position == -1:
            stop_loss     = entry_price + (atr_mult * atr)   # fixed SL direction
            target_profit = entry_price - (3 * atr)          # fixed TP direction
            if close <= target_profit or (close >= stop_loss) or macd_cross_up:
                exit_price = close
                ...
                trade_log.append({
                    ...
                    'pnl': pnl,
                    'bars_held': i - entry_index,
                    'exit_reason': 'MACD_cross' if macd_cross_up else 'ATR/TP',
                    'rsi_exit': rsi,
                    'ema_rsi_exit': ema_rsi,
                    'macd_exit': macd,
                    'signal_exit': signal,
...
if match:
    ...
    print(f"\n\033[1mTrading Performance Summary for {product} {timeframe} (MACD+RSI EMA filter + ATR 1.5x with crossover exits):\033[0m")
else:
    print("\n\033[1mTrading Performance Summary (MACD+RSI EMA filter + ATR 1.5x with crossover exits):\033[0m")
