...
# ==================== HEDGE RATIO ====================
WINDOW = 50  # rolling window size for hedge ratio calculation
...
SHORT:
    ...
    pnl = -(CL_exit - CL_entry) * CL_MULT
          + h * (BR_exit - BR_entry) * BR_MULT
...
# ==================== EXCEL EXPORT ====================
with pd.ExcelWriter("CL_BR_Copula_FINAL.xlsx", engine="openpyxl") as writer:
    trades_df.to_excel(writer, sheet_name="Trades", index=False)
    summary_df = pd.DataFrame(list(summary.items()), columns=["Metric", "Value"])
    summary_df.to_excel(writer, sheet_name="Summary", index=False)

print("\nExcel saved: CL_BR_Copula_FINAL.xlsx")
