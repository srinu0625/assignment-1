...
# CONFIG
# ==========================
CSV_FILE    = r"D:\data 3\CL_BR 30.csv"
OUTPUT_PATH = r"D:\ARB_PROJECT1\CL_BR_UPDATED___30min_____==_.xlsx"
...
TCOST = 20.0  # FIXED: Realistic cost for CL+BR round turn
...
for ENTRY_Z in Z_LIST:
    ...
    for i in range(ROLLING_WINDOW, len(df)):
        ...
        # FIXED: Proper cointegration - Regress CL on BR
        ...
        # Dollar neutral: if Long CL, Short BR -> BR contracts = CL contracts * beta * (CL_MULT/BR_MULT)
        dollar_hedge_ratio = abs(beta) * (CL_MULT / BR_MULT)

        if position == 0:
            # FIXED: Entry on CROSSINGS, not levels
            # Short CL Long BR: when Z crosses UP through ENTRY_Z (overbought)
            if z_prev < ENTRY_Z and z >= ENTRY_Z:
                position = -1
                entry = (date, cl_price, br_price, bar_count, "SHORT", dollar_hedge_ratio, beta)
                print(f"{date} | \033[91mSHORT CL LONG BR\033[0m | Z={z:.2f} | Beta={beta:.3f} | HL={half_life:.1f}")
                time.sleep(3)  # Reduced sleep for faster backtest

            # Long CL Short BR: when Z crosses DOWN through -ENTRY_Z (oversold)
            elif z_prev > -ENTRY_Z and z <= -ENTRY_Z:
                position = 1
                entry = (date, cl_price, br_price, bar_count, "LONG", dollar_hedge_ratio, beta)
                print(f"{date} | \033[92mLONG CL SHORT BR\033[0m | Z={z:.2f} | Beta={beta:.3f} | HL={half_life:.1f}")
                time.sleep(3)
