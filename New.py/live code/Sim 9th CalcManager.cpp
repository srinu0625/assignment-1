...
bool CalcManager::TradingTimings( std::string Symbol ) 
{
	...
	if((Symbol == "MESU4") || (Symbol == "MNQU4") || (Symbol == "MYMU4") || (Symbol == "MGCQ4") || (Symbol == "MBTN4"))       //  || (Symbol == "YMZ3")
	{
...
bool CalcManager::bullish()
{
	...
	try
	{
		...
		if((lastTradedPrice > local_high1) && ((local_low1 > local_high2) || (local_low1 > local_high3) || (local_low1 > curr_high3))) condition1 = true;

		bool condition2 = false;
		if(lastTradedPrice > curr_high1) condition2 = true;

		bool condition3 = false;
		if((local_low1 > local_low2) && (local_low1 > local_low3) && (local_low1 > curr_low3)) condition3 = true;
		...
		if( condition1 && condition2 && condition3 && condition4 && (bearFirstTrigg == false))
		{
			...
			else if(bullFirstTrigg==true && m_BidAskData.ask > prevBuyOrderPrice)
			{
...
bool CalcManager::exitbullishposition()
{
	try
	{
		...
		{
			std::cout << GetSymbol() << " Long Exit Algo4" << " myPosition= " << myPosition << std::endl;
			numSell++;
			SendNewOrder(m_BidAskData.bid - (10*tickSizeNew),exchange::rom::Sell, myPosition);
			bullFirstTrigg = false;
			return true;
...
bool CalcManager::bearish()
{
	...
	try
	{
		...
		bool condition1 = false;
		if((lastTradedPrice < local_low1) && ((local_high1 < local_low2) || (local_high1 < local_low3)  || (local_high1 < curr_low3))) condition1 = true;

        bool condition2 = false;
		if(lastTradedPrice < curr_low1) condition2 = true;

		bool condition3 = false;
		if((local_high1 < local_high2) && (local_high1 < local_high3) && (local_high1 < curr_high3)) condition3 = true;
		...
		if(condition1 && condition2 && condition3 && condition4 && (bullFirstTrigg == false))            
		{
			...
			else if(bearFirstTrigg == true && m_BidAskData.bid < prevSellOrderPrice)
			{
				CancelExistingLiveOrders();
...
bool CalcManager::exitbearishposition()
{
	try
	{
		...
		{
			numBuy++;
			std::cout << GetSymbol() << " Short Exit Algo4" << " myPosition= " << myPosition << std::endl;
			SendNewOrder(m_BidAskData.ask + (10*tickSizeNew),exchange::rom::Buy, (-1*myPosition));                       
			bearFirstTrigg = false;
			return true;
		}
...
void CalcManager::OnLastPriceUpdate()
{
	...
	try
	{
		...
		else if(GetSymbol() == "MYMU4")
		{
			priceVal = 0.5;
			tickSizeNew = 1;
		}
		else if(GetSymbol() == "MNQU4") 
		{
			priceVal = 2;
			tickSizeNew = 0.25;
		}
		else if(GetSymbol() == "MCLQ4") 
		{
			priceVal = 100;
			tickSizeNew = 0.01;
		}
		else if(GetSymbol() == "MBTN4") 
		{
			priceVal = 0.1;
			tickSizeNew = 5;
		}
		else if(GetSymbol() == "MGCQ4") 
		{
			priceVal = 10;
			tickSizeNew = 0.1;
		}
...
void CalcManager::FormCandle()
{
	try
	{
		...
		if(((tm_struct->tm_min%timeSlot == 0) || (tm_struct->tm_hour == 13 && tm_struct->tm_min == 35)) && flag == true)
		{
			...
			// tm_struct->tm_min = 30;
			if(tm_struct->tm_hour == 13 && tm_struct->tm_min == 35)
			{
				pos_prev_local_high1 = curr_high1;
				pos_local_high1 = curr_high1;
				pos_prev_local_low1 = curr_low1;
				pos_local_low1 = curr_low1;
				temp_high1 = curr_high1;
                temp_low1 = curr_low1;
			}
			...
			std::cout << "Curr High3 " << curr_high3 << "  Curr Low3 " << curr_low3 << " Prev high3 " << prev_high3 << " Prev Low3 " << prev_low3 << " Temp High3 " << temp_high3 << " temp_low3 " << temp_low3  << " Local High3 " << local_high3 << " Local low3 " << local_low3 << " Prev Local High3 " << prev_local_high3 << " Prev Local low3 " << prev_local_low3 << std::endl << std::endl;
		...
		if((tm_struct->tm_min%timeSlot == 1) || (tm_struct->tm_min%timeSlot == 6))
		{
			flag = true;
		}
	...
	// update high2, low2 for first feed.
		...
		if((tm_struct->tm_min%60 == 0) && flag2 == true)         //(tm_struct_chicago->tm_hour - 1 + 4)
		{
			//Implementation of Inside Candle
	...
	//######################## FOR 3rd CANDLE(Daily)

	// update high3, low3 for first feed.
		if(firstFeed3 == true)
		{
			high3 = lastTradedPrice;
			low3 = lastTradedPrice;
			firstFeed3 = false;
		}

		if(high3 == 0) high3 = lastTradedPrice;
		if(low3 == 0) low3 = lastTradedPrice;
		
		// Logic for calculate high3 , low3.
		if(lastTradedPrice > high3) high3 = lastTradedPrice;
		if(lastTradedPrice < low3) low3 = lastTradedPrice;

		if(high3 > curr_high3)
		{
			if(flagFlyChange13) prev_local_low3 = pos_local_low3;
			flagFlyChange13 = false;
			local_low3 = temp_low3;
		}
		if(low3 < curr_low3)
		{
			if(flagFlyChange23) prev_local_high3 = pos_local_high3;
			flagFlyChange23 = false;
			local_high3 = temp_high3;
		}

		if(curr_high3 == 0 || curr_low3 == 0)
		{
			curr_high3 = high3;
			curr_low3 = low3;
			temp_high3 = high3;
		...
		if((tm_struct->tm_hour == 21) && flag3 == true)
		{
			//Implementation of Inside Candle
			if((high3>curr_high3) || (low3<curr_low3))
			{
				prev_high3 = curr_high3;
				prev_low3 = curr_low3;
				curr_high3 = high3;
				curr_low3 = low3;
				newFlag3 = true;
			}
			if((curr_high3 > prev_high3) && (curr_low3 < prev_low3))
			{
				temp_high3 = curr_high3;
				temp_low3 = curr_low3;
				if(newFlag3)
				{
					pos_prev_local_low3 = pos_local_low3;
				    pos_prev_local_high3 = pos_local_high3;
					newFlag3 = false;
				}
			}

			if( curr_high3 > prev_high3)
			{
				if((curr_high3 > prev_high3) && (curr_low3 < prev_low3))
				{
					posflag13 = false;
				...
				flagFlyChange23 = true;
				posflag13 = false;
				posflag23 = true;

				pos_local_low3 = temp_low3;
			}
			if(curr_low3< prev_low3)
			{
				if((curr_high3 > prev_high3) && (curr_low3 < prev_low3))
				{
					posflag13 = false;
				...
				flagFlyChange13 = true;
				posflag23 = false;
				posflag13 = true;

				pos_local_high3 = temp_high3;
			}
			...
			local_high3 = pos_local_high3;
			prev_local_high3 = pos_prev_local_high3;
			local_low3 = pos_local_low3;
			prev_local_low3 = pos_prev_local_low3;
		
			std::cout << GetSymbol() << " Candle3: Big Candle " << current_Time  << "  Local High3 : " << local_high3 << "  Local low3 : " << local_low3 << "Current High3 : " << curr_high3 << "  Current Low3 : " << curr_low3 << std::endl;
			//std::cout << endl;
			high3 = lastTradedPrice;
			low3 = lastTradedPrice;
			flag3 = false;
