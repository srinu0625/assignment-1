...
# --------------------------------------------
TIMEFRAME = "240"      # 5,10,15,30,60,240 or "D"
...
def sequential_backtest(df):
    ...
    for i in range(1, len(df)):
            ...
            if position["side"] == "long":
                ...
                for idx, tp in enumerate(position["tp_prices"]):
                    if not position["tp_hit_flags"][idx] and row["HIGH"] >= tp:
                        ...
                        if idx == 1: position["stop"] = position["tp_prices"][0]
        ...
        if (
            pd.isna(row["ATR"]) 
            or pd.isna(row["MA24"]) 
            or pd.isna(prev["HH50"]) 
            or pd.isna(prev["LL50"])
        ):
            continue
