...
int CalcManager::calRisk1()
{
	try
	{
		...
		double diff = 0;
		diff = m_BidAskData.ask - local_low1;
...
int CalcManager::calRisk2()
{
	try
	{
		...
		double diff = 0;
		diff = local_high1 - m_BidAskData.bid;
...
int CalcManager::calRisk12()
{
	try
	{
		...
		double diff = 0;
		diff = prevBuyOrderPrice - local_low1;
...
int CalcManager::calRisk22()
{
	try
	{
		...
		double diff = 0;
		diff = local_high1 - prevSellOrderPrice;
...
bool CalcManager::bullish()
{
	...
	try
	{
		...
		if((lastTradedPrice > local_high1) && (local_high1 > local_high2)) condition1 = true;

		bool condition2 = false;
		if(lastTradedPrice > curr_high1) condition2 = true;

		bool condition3 = false;
		if((local_low1 >= local_low2)  && ((local_high1 > local_high2) || (local_high1 > curr_high2))) condition3 = true;
...
bool CalcManager::bearish()
{
	...
	try
	{
		...
		bool condition1 = false;
		if((lastTradedPrice < local_low1) && (local_low1 < local_low2)) condition1 = true;

        bool condition2 = false;
		if(lastTradedPrice < curr_low1) condition2 = true;

		bool condition3 = false;
		if((local_high1 <= local_high2) && ((local_low1 < local_low2) || (local_low1 < curr_low2))) condition3 = true;
		...
		if(condition1 && condition2 && condition3 && condition4 && (bullFirstTrigg == false))            
		{
			...
			if(bearFirstTrigg == false)
			{
                ...
                numSell++;
				bearFirstTrigg = true;
				ShortOrderExitPrice = local_high1;
