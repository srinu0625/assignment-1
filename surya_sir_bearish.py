...
def is_bearish_entry(row):
    ...
    if open_price > close_price:
        open_low_ratio = (high_price - open_price) / (high_price - low_price)
        close_open_ratio = (open_price - close_price) / (high_price - low_price)
        high_close_ratio = (close_price - low_price) / (high_price - low_price)

        if (0.39
                <= open_low_ratio <= 0.59 and 0.39 <= close_open_ratio <= 0.59 and high_close_ratio <= 0.12):
            return True
    return False
    ...
    if is_bearish_entry(row):
        ...
        for i in range(index + 1, len(df)):
            ...
            if current_price <= take_profit:
                pnl = (entry_price - take_profit) * 1 * 50
