...
for i in range(max(atr_period, 200), len(df)):
    try:
        ...
        # ---------------- Long Entry ----------------
        if position == 0 and  (macd > signal) and ((macd - signal) > macd_gap_min) and (rsi > 55) and (ema50 > ema200) and (close > ema50) \
            and (atr > atr_median):
            position = 1
            entry_price = close
        ...
        if position == 1:
            ...
            # ---------------- Short Entry ----------------
            if position == 0 and (macd < signal) and ((signal - macd) > macd_gap_min) and (rsi < 45) and (ema50 < ema200) and (close < ema50) \
                and (atr > atr_median):
                position = -1
                entry_price = close
                tp_price = entry_price - profit_atr * atr
...
if trade_log:
    ...
    try:
        ...
        print(f"D:\\trade logs: {output_path}")
    except Exception as e:
