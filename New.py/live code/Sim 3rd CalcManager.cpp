...
bool CalcManager::bearish(int slot)
{
	...
	try
	{
		...
		if(slot == 5)
		{
			...
			if(min(low12, min(low13, min(low14, low15))) > low1) condition2 = true;
			...
			if(condition1 && condition2 && longEntry1)
			{
				...
				entryPrice = close1;
				stop1 = max(high12, max(high13, max(high14, high15)));
				targetRange = stop1-low1;
				targetRange1 = stop1-low1;
				profPrice = close1 - targetRange;
				std::cout << "5min Short Entry   EntryPrice:" << entryPrice << "  TargetRange:" << targetRange << " profPrice:" << profPrice << std::endl;
				SendNewOrder(entryPrice - 10*tickSizeNew,exchange::rom::Sell, numLot, 5, 2);
				SendNewOrder(profPrice,exchange::rom::Buy, numLot, 5, 1);
				stop1 = high1;
				shortEntry1 = false;
			}
		}
...
bool CalcManager::exitbearishposition()
{
	try
	{
		...
		if(myPosition != 0 && lastTradedPrice > stop1 && shortExit1)
		{
			shortExit1 = false;
			CancelExistingLiveOrders(5);
			SendNewOrder(stop1+10*tickSizeNew,exchange::rom::Buy, numLot, 5, 2);
			stop1 = INT16_MAX;
		}
