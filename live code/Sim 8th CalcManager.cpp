...
bool CalcManager::TradingTimings( std::string Symbol ) 
{
	...
	if((Symbol == "MESZ4") || (Symbol == "MNQZ4") || (Symbol == "MYMZ4") || (Symbol == "MGCZ4") || (Symbol == "MBTX4"))       //  || (Symbol == "YMZ3")
	{
		start_hour = 01;   //5:30am->6:30am
		start_min = 00;
		end_hour = 21;    //1:30am->2:30
		end_min = 00;
	}

	if((Symbol == "MCLZ4"))
	{
		start_hour = 01;   // 6:30am
		start_min = 00;
		end_hour = 20;    // 01:30am
		end_min = 00;
	}
...
int CalcManager::calRisk1()
{
	try
	{
		...
		if(maxLoss > 720) return 1;
		if(maxLoss == 0) return 0;

        int riskQuant = static_cast<int>(round(720/LotPrice));
		
        if(riskQuant > 20)
		{
			riskQuant = 20;
		} 
		if(riskQuant > 12 && GetSymbol() == "MGCZ4")
		{
			riskQuant = 12;
		}
		if(riskQuant > 15 && GetSymbol() == "MBTX4")
		{
			riskQuant = 15;
		}
...
bool CalcManager::bullish()
{
	...
	try
	{
		...
		if((local_low1 >= local_low2)) condition3 = true;
...
bool CalcManager::bearish()
{
	...
	try
	{
		...
		bool condition3 = false;
		if((local_high1 <= local_high2)) condition3 = true;
...
void CalcManager::TradeSettlement()
{
	try
	{
		...
		if((settlementFlag == true) && (GetSymbol() == "MCLZ4") && (myPosition!=0) && (current_hour == 19 && current_minute >= 00))
		{
			settlementFlag = false;
			if(myPosition > 0)
			{
...
void CalcManager::OnLastPriceUpdate()
{
	...
	try
	{
		...
		else if(GetSymbol() == "MCLZ4") 
		{
			priceVal = 100;
			tickSizeNew = 0.01;
		}
		else if(GetSymbol() == "MBTX4") 
		{
			priceVal = 0.1;
			tickSizeNew = 5;
		}
		else if(GetSymbol() == "MGCZ4") 
		{
			priceVal = 10;
			tickSizeNew = 0.1;
		}
...
void CalcManager::OnOrderSubmittedFromSim(exchange::rom::ROMStatusMsg& msg)
{
    ...
    std::string subTag = msgTag.substr(14);
...
void CalcManager::FormCandle()
{
	try
	{
		...
		if((((tm_struct->tm_hour + 1)%4 == 0)) && flag == true)         //(tm_struct_chicago->tm_hour - 1 + 4)
		{
	...
	// update high2, low2 for first feed.
		...
		if((tm_struct->tm_hour == 22) && flag2 == true)
		{
			//Implementation of Inside Candle
